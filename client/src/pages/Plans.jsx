...
const planIcons = { 'Starter': '🌱', 'Professional': '🚀', 'Enterprise': '🏢' };
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 relative z-10">
        ...
        {mySub && mySub.status === 'active' && (
          ...
          >
            ...
            <div className="text-4xl mb-3">{planIcons[mySub.planName] || '⭐'}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {localPlans.map((plan, i) => (
              <motion.div key={plan.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.1 }}
                whileHover={{ y: -8 }}
                className={`relative rounded-2xl p-6 border transition-all duration-300 flex flex-col overflow-hidden group ${
                  plan.badge
              ...
              >
                ...
                <div className="relative">
                  <div className="text-3xl mb-3">{planIcons[plan.name] || '📋'}</div>
                  {plan.badge && (
                    <motion.span
                      animate={{ y: [0, -2, 0] }}
                      transition={{ repeat: Infinity, duration: 2 }}
                  ...
                  <motion.button onClick={() => handleSubscribe(plan)} disabled={subscribing === plan.id}
                    whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
