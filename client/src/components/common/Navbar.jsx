...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-background/90 backdrop-blur-xl border-t border-electric-violet/10 pb-1">
          ...
          {[
            ...
            if (tab.toggle) {
              return (
                <button key={tab.label} onClick={() => setMobileOpen((prev) => !prev)}
                  className={`flex flex-col items-center gap-0.5 px-5 py-1 rounded-lg text-[10px] transition-all ${mobileOpen ? 'text-neon-cyan' : 'text-text-muted'}`}
                >
            ...
            return (
              <Link key={tab.label} to={tab.path} onClick={() => setMobileOpen(false)}
                className={`flex flex-col items-center gap-0.5 px-5 py-1 rounded-lg text-[10px] transition-all ${
                  tab.highlight
                    ? 'bg-gradient-to-r from-neon-cyan/15 to-electric-violet/15 border border-neon-cyan/30 text-neon-cyan -mt-1 px-6 py-1.5'
                    : isActive
                      ? 'text-neon-cyan'
